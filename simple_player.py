...
class SimplePlayer(AbstractPlayer):
    ...
    def get_move(self, board, possible_moves):
        ...
        for i, move in enumerate(moves):  # possible moves
            ...
            if len(self.game.rules.get_move_list(temp_board, Disk(3 - self.color.value))) == 0:  # if after the update the other player has no moves
                if len(self.game.rules.get_move_list(temp_board, self.color)) == 0:  # see if this player has no moves also
    ...
    def find_score(self, board):
        dark_count, white_count = self.game.rules.count_pieces(board, board.board_size())
